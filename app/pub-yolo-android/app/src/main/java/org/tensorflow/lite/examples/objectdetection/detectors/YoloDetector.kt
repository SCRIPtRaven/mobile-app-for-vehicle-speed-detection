...
class YoloDetector(
    ...
    override fun detect(image: TensorImage, imageRotation: Int): DetectionResult  {
        ...
        for (result: DetectedObject in results) {
            // Respect the configured maxResults: stop collecting once we've reached it
            if (detections.size >= maxResults) break

            // Normalize label and accept a set of vehicle labels
            val rawLabel = result.label?.lowercase() ?: continue
            val vehicleLabels = setOf("car", "truck", "bus", "motorcycle", "motorbike", "van", "lorry", "suv", "pickup", "minivan")
            if (!vehicleLabels.contains(rawLabel)) continue
